...
void prepareTerrain() {
	...
	for (int i = 1; i < imageWidth - 2; i++) {
		for (int j = 1; j < imageWidth - 1; j++) {
			...
			texCoord.push_back((i+1) - imageWidth * 0.5f + 0.5f);
			texCoord.push_back(j - imageWidth * 0.5f + 0.5f);
			...
			texCoord.push_back(i - imageWidth * 0.5f + 0.5f);
			texCoord.push_back(j - imageWidth * 0.5f + 0.5f);
...
void loadTexture() {
	...
	gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, tw, th,
