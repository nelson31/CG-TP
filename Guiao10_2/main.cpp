...
void computeNormal(int i, int j) {
	...
	/* Pontos auxiliares para 
	calcular a normal */
	float p1[3], p2[3], p3[3], p4[3];
	/* Vetores auxiliares para calcular 
	as normais */
	float v1[3], v2[3];
	/* Vetor que irá conter a normal 
	para o ponto (i,j)*/
	float norm[3];
	/* Construção dos diferentes pontos 
	auxiliares */
	p1[0] = i; p1[1] = h(i, j - 1); p1[2] = j - 1;
	p2[0] = i; p2[1] = h(i, j + 1); p2[2] = j + 1;
	p3[0] = i - 1; p3[1] = h(i - 1, j); p3[2] = j;
	p4[0] = i + 1; p4[1] = h(i + 1, j); p4[2] = j;
	/* Contrução dos diferentes vetores 
	...
	normalize(norm);
	/* Adicionamos os valores do vetor 
	obtido ao vetor de normais */
	for (int i = 0; i < 3; i++)
		normal.push_back(norm[i]);
}
